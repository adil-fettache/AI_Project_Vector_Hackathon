...
class AgentExecutor(A2AAgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        # Load MCP tools if user token is available (or in local testing mode)
        tools = []
        is_local_testing = os.environ.get("IBD_TESTING") == "1"

        if not user_token and not is_local_testing:
            logger.warning("No user token available - running agent without tools")
        else:
            try:
                # In local testing mode, user_token can be None/empty
